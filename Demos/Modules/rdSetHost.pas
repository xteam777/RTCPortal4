...
uses
  ...
  Classes, Graphics, Controls, Forms, rtcSystem,
...
procedure TrdHostSettings.Setup;
  ...
  if assigned(PDesktop) then
    ...
    if PDesktop.MirrorDriverInstalled then
      ...
      btnMirrorUninstall.Enabled:=File_Exists(ExtractFilePath(ParamStr(0))+'\VideoDriver\Uninstall.bat');
      end
    else
      begin
      lblDriverCheck.Caption:='(driver NOT installed)';
      btnMirrorInstall.Enabled:=File_Exists(ExtractFilePath(ParamStr(0))+'\VideoDriver\Install.bat');
